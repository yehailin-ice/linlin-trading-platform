...
风险提醒：本项目仅用于个人交易记录、复盘和辅助分析，不构成任何投资建议。模型和自动交易规则仍需持续优化，行情接口可能延迟或失效，交易有风险，请不要盲目使用，更不要直接依据模型输出进行实盘交易。
</p>
...
- <span style="color: red; font-weight: 700;">交易有风险，入市需谨慎。本项目不构成投资建议，也不承诺任何收益。</span>
- <span style="color: red; font-weight: 700;">模型、评分、候选股、AI 自动交易逻辑仍需持续优化，请不要盲目使用，不要直接依据模型输出进行实盘交易。</span>
